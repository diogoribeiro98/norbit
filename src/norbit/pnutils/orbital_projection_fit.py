...
def get_sky_projection_fit(
    ...
    for itt in np.arange(0,len(solution.t)):
        ...
        t  = solution.t[itt]
        r_star = vec3([solution.y[0, itt],solution.y[1, itt],solution.y[2, itt]])
        v_star = vec3([solution.y[3, itt],solution.y[4, itt],solution.y[5, itt]])
        
        #Get light reception angle and corresponding time delay
        deltat, light_vec = ode.deflection_position( r_star ,r_observer , pncor=light_pncor)

        #Get corrected velocity (with redshift)
        v_redshift = (redshift_prefactor(r_star,r_observer, v_star, vec3([0,0,v_observer*1000/units.c]), metric)*ode.dtdt0(r_star,r_observer,v_star,vec3([0,0,v_observer*1000/units.c]))-1)
